...
$137$ time slices and each time slice had the signal amplitudes of over 
$100,000$ voxels. The voxels were referred using the 3D coordinates and thus 
the dimensionality of a single run was 
$\mathcal R^{53\times N64\times N37 \times 137}$, where $137$ corresponds to the number of time slices.
...
hyper parameters of the system using five-hold cross validations. After finding 
...
\caption{Voxel ranking methods to select best voxels. In the second step the differences between the class averages of degrees between patients and healthy subjects are $2$,$-1$ and $-3$ respectively for three voxels.}
\end{figure}
...
as the smoothed log values (adding a small positive number to the degree of each voxel before getting the log value) of the voxels for each subject. Furthermore, as 
...
These include \begin{enumerate}
  \item Selecting voxels based on two-tailed t-test.
  \item Selecting voxels based on absolute differences between the mean degrees of a voxel between schizophrenic and healthy subjects.
  \item Selecting voxels based on differences between the mean degrees of a voxel between schizophrenic and healthy subjects.
  \item Selecting voxels based on differences between the mean degrees of a voxel between healthy and schizophrenic subjects.
\end{enumerate}
...
\subsection{SGMRF with degrees of voxels}
With the Rish \emph{et al.} code we obtained a $69.82\%$ cross validation 
accuracy on our cross validation sets and $72.32\%$ accuracy on the hold-out 
set.
\subsection{SGMRF with degrees from DLPFC}
We obtained an accuracy of $63.52\%$ using the log degrees in the voxels of DLPFC.
\subsection{Methods using Power \emph{et al.}'s ROIs}
...
One of the interesting result we observed is that the method to select the best voxels had a significant impact on the accuracy. Use of the mean difference instead of t-test or the absolute mean difference proved to be more efficient in differentiating patients and healthy subjects. Specifically, use of the degree differences between the patients and the healthy subjects showed higher accuracy. This corresponds to the use of increased connectivities in the brain in schizophrenia patients for classification. The hyper-response in BOLD signal in schizophrenia patients is observed in \cite{Potkin2008}.

Also use of degrees from DLPFC and thalamus areas of the brain resulted in $63.52\%$ accuracy. Although this value is significantly higher than the baseline accuracy of $52\%$, other methods resulted in higher classification accuracies. We can conclude that connectivities in DLFPC and thalamus areas are affected during schizophrenia, but those were not the only affected areas. Also, further research should be carried out to identify whether the selected best voxels for SGMRF with log degrees were the same for different cross validation sets and whether those voxels represented a specific region of the brain.
