...
usediag=0; % 0 - regularize diagonal, 1 - "regularized likelihood"
    ...
    sumtabf = 'res_sum_table_l_fixed_rand_p100sp1_SINCO.txt';
    tabf = 'res_table_l_fixed_rand_p100sp1_SINCO.txt';  
    fid_sum_table = fopen(sumtabf,'a');
    fprintf(fid_sum_table,'%% N density  lambda_avg LLtest TP FP \n');   
...
Nvec = [50 100 300 500 1000 5000 10000 100000];
%spvec = [3 5 7];
spvec= [1 2 3];
...
for  ni = 1:length(Nvec) % number of samples
  ...
  for si=1:1
        ...
        for mats=1:5
                runs1 = runs1+1;
      %A=load(sprintf('../Random/mat%dpar%d.txt',mats,spvec(si)));
        A=load(sprintf('../Random/p%dmat%dpar%d.txt',p,mats,spvec(si)));
                ...
                if (ifvec)            
                    ...
                    fprintf(fid_table,'%d %.2f   %f %f %f %f\n', N, density,   our_lambda, test_ll, TP, FP);
                end
                %keyboard
        end % mats                
