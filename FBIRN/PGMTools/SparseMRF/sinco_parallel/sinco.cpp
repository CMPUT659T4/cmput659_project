...
/* This is the C++ implementation to solve the sparse inverse covariance
   problem of the form
...
(not necessarily symmetric), S.*C is a elemwhise product and ||*||_1 is the sum
 ...
 is slow in the end.  */
  ...
  while (!stop && (iter<100000)) {
    // %find the largest positive derivative 
      ...
      /*         for (int i=0; i<p; i++) { 
      for (int j=0; j<p; j++) {
        fprintf(pFile, "\n [%8f] [%8f] [%4d] [%4d]", AllSteps(i,j).fchange, AllSteps(i,j).alpha, i, j);
       }
       } */
      // fprintf(pFile, "\n [%6f] [%6f] [%6f] [%6f]", funmax-f, Gradp(imax,jmax), Gradpp(imax,jmax), alphamax);
         if ( (alphamax==0) || ((funmax-f)/(fabs(f)+1))<tol ) { stop=true;}
          
      /*      if ( (fabs(alphamax)<tol*(1+Cp(imax,imax)+Cp(jmax,jmax)) )) { stop=true;
	      fprintf(pFile, "\n [%6f] [%6f] [%6f] [%6f]", funmax-f, Cp(imax,imax), Cp(imax,imax), alphamax); } */
    else {
      if (updatemax==1) {
...
/*     matrix C=Cp;
      C.sumwithscal(-1, Cpp);
      double normC=C.normS(S);
      double trace=C.dotmultiply(A); */
...
int main()
{
  /* the main routine for testing in C++ environment */
  double v1, v2;
  ICS test;
  test.p=200;
  int p=test.p;
  ...
  for (int i=0; i<p; i++) { 
    for (int j=0; j<p; j++) {
      A(i,j)=(1.0*(rand() % 100))/100.0;
    }
  }
  ifstream myfile ("ECmat.txt");
  /*  myfile.open(); */
  for (int i=0; i<p; i++) { 
    for (int j=0; j<p; j++) {
      myfile>>test.A(i,j);
