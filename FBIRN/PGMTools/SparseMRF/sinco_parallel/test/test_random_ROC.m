...
usediag=0; % 0 - regularize diagonal, 1 - "regularized likelihood"
    ...
    sumtabf = 'res_sum_table_ROC_rand_p200sp1_SINCO_1_tol-6.txt';
    tabf = 'res_table_ROC_rand_p200sp1_SINCO_1_tol-6.txt';  
    timetabf = 'res_time_ROC_rand_p200sp1_SINCO_1_tol-6.txt';  
    
    fid_sum_table = fopen(sumtabf,'a');
    fprintf(fid_sum_table,'%% N density lambda LLtest TP FP cputime\n'); 
    fid_table = fopen(tabf,'a');
    fprintf(fid_table,'%% N density lambda LLtest TP FP cputime\n');
    fid_time_table = fopen(timetabf,'a');
    fprintf(fid_time_table,'%% N density cpu_time_total\n');
...
lambda_range = [500 450 400 350 300 250 200 100 50 40 30 20 10 5 1];
 
%c(30, 40, 50, 60, 70, 80, 90, 100, 200, 250, 300, 350, 400, 450, 500)
  ...
  for si=1:1
      ...
      for mats=1:5
          ...
          for lambda=lambda_range
                ...
                t=cputime;
                [Csol, Wsol, fsol]=...
                    sinco_matlab(Cstart, fstart, Wstart, EC, Sbase, lambda, K, tol);
                cpu_time(i)=cputime-t;
                cpu_time_total = cpu_time(i) + cpu_time_total;
...
%                 tra(i)=trace(EC*Csol);
                ...
                fprintf(fid_table,'%d %.2f %f  %f %f %f %f\n', N, density, lambda, test_ll, TP(i), FP(i),cpu_time(i));
                
          end % for lambda
                %'%% N density user_time system_time elapsed_time\n
                fprintf(fid_time_table,'%d %.2f %f  \n', N, density, cpu_time_total);
      end % mats                
