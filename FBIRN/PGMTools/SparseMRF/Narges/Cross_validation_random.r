...
file_name<-"res_CV_random.txt"
fnm_table<-"res_table_CV_random.txt"
fnm_sum_table<-"res_sum_table_CV_random.txt"

write("%n density lambda_avg lse TP FP",fnm_sum_table,append=TRUE);
write("%n density_avg lambda lse TP FP ",fnm_table,append=TRUE);

seed_r<-c(3,97,12,32,42,52,62,72,82,92,102,100,200,300,500,555,546,223,999,597,97,99,888,832,732,777,735,733,723,713,703,603)
...
write("----------------------------------------------------------",file_name,append=TRUE)
 ...
 write(FP_avg/runs1,file_name,append=TRUE)
		...
		tp_avg = TP_avg/runs1;
		fp_avg = FP_avg/runs1;
		res<-c(dd,density_avg,l_avg,lse_avg,tp_avg,fp_avg);
