...
#file_name<-"results_Notscaled_exponential_vector.txt"

file_name<-"res_exp_vector_SF_same_b.txt"
fnm_table<-"res_table_exp_vector_SF_same_b.txt"
fnm_sum_table<-"res_sum_table_exp_vector_SF_same_b.txt"
	
write("%n density b lambda_avg lse TP FP",fnm_sum_table,append=TRUE);
write("%n density b lambda lse TP FP ",fnm_table,append=TRUE);

N_vector<- c(30,50,150,250,500,1000)
b_vector<-c(1/0.00001,1/0.00005,1/0.0001,1/0.0005,1/0.001,1/0.005,1/0.01);
...
b_old<-rep(0,ss)
for(i in 1:ss){
b_old[i]<-sum(abs(Sinv[i,]))
}
b_old<-b_old/(ss)
#b<-b*(ss/dd)

b<-rep(bb,ss) # Modified by Irina set same b but allow vector-lambda optimization
...
write("FP of the learned C ",file_name,append=TRUE)
 write(fp,file_name,append=TRUE)
		...
		res<-c(dd,density,bb,glcalls,lse,tp,fp);
		write(res,fnm_table,7,append=TRUE);
...
write("----------------------------------------------------------",file_name,append=TRUE)
 ...
 write(FP_avg/runs1,file_name,append=TRUE)
		...
		tp_avg = TP_avg/runs1;
		fp_avg = FP_avg/runs1;
		res<-c(dd,density,bb,glcalls_avg,lse_avg,tp_avg,fp_avg);
		write(res,fnm_sum_table,7,append=TRUE);
