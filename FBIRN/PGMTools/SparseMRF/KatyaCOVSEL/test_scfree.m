...
% nzfrac = 0.025;
% nnz=nzfrac*p*p;          % Number of nonzero coefficients in A
% 
% 
% precision = 0.005;
% lstart = 10;
...
b= sum(abs(inv(EmpCov + 0.001*eye(p,p))))/(p-1);
...
lstart=100*(ones(p,1));
 [C,our_lambda] = AltMin_vec(EmpCov, N, Sbase, b, precision, tol, lstart);
% our_lambda
 keyboard
% i=0;
...
% our_true_neg = size(find(~our_C  & ~A),1);
% %     
