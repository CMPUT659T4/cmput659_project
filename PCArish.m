...
X_train=Train(:,1:(end-1)/1);
size(X_train)
pc_train=ppca(X_train',70);
Y_train=Train(:,end);
X_test=Test(:,1:(end-1)/1);
pc_test=ppca(X_test',70);
Y_test=Test(:,end);
...
for lambda1=lambdas
    ...
    model = MRFC_learn(pc_train, Y_train, method, lambda1);  %learn MRF classifier (MRFC)
    predicted_Y = MRFC_predict(pc_test,model); %test MRFC
