function [Accuracy,PErrVec,FErrVec,AnalyseMatrix]=CombinedNaiveBayesCrossValidation2(limit,limit2,itterat)
...
for i=1:itterat
    ...
    data2=OurTestMFG_SFGVoxelsLogDegrees;
    ...
    for k=1:5
        ...
        [predicted_Y1 Probs] = MRFC_predict(X_test,model); %test MRFC
...
%         
        ...
        X_train=Train(:,1:limit2);
        Y_train=Train(:,end);
        X_test=Test(:,1:limit2);
        Y_test=Test(:,end);
        ...
        model = MRFC_learn(X_train, Y_train, method, lambdas);  %learn MRF classifier (MRFC)
        [predicted_Y2 Probs] = MRFC_predict(X_test,model); %test MRFC
        
        predicted_Y=zeros(size(predicted_Y2));
        for mnh=1:length(predicted_Y)
            if predicted_Y1(mnh)==-1
                predicted_Y(mnh)=-1;
            else
                if predicted_Y1(mnh)==1 && predicted_Y2(mnh)==1 
                    predicted_Y(mnh)=1;
                else
                    predicted_Y(mnh)=-1;
                end
